      ...
      <section className="zl-wrap" style={{ paddingTop: 40, paddingBottom: 80 }}>
        ...
        <div style={{ display: "flex", gap: 24, alignItems: "flex-start", marginBottom: 40, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            {album.description && <p style={{ color: "var(--text-3)", fontSize: "0.85rem", margin: "8px 0 0", maxWidth: 480 }}>{album.description}</p>}
            <div style={{ display: "flex", gap: 10, marginTop: 16, flexWrap: "wrap" }}>
              <a
                href={`/dashboard/upload?album=${album.id}`}
        ...
        {tracks.length === 0 ? (
          <div className="zl-card" style={{ padding: "48px 32px", textAlign: "center" }}>
            ...
            <p className="zl-muted" style={{ marginBottom: 20 }}>Este álbum no tiene tracks aún.</p>
            <a href={`/dashboard/upload?album=${album.id}`} className="zl-btn zl-btn--primary">
              + Añadir primer track
            </a>
          </div>
        ) : (
